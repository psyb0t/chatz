...
- **Just OpenAI** — set `OPENAI_API_KEY`. With `CHATZ_UPSTREAMS` empty, the app
  falls back to a built-in OpenAI upstream keyed on that variable.
- **Any supported endpoint, including multiple providers** — set
  `CHATZ_UPSTREAMS` to a JSON array. `provider` is `openai` for OpenAI and
  OpenAI-compatible services, or `anthropic` for Anthropic. API keys are
  referenced by env-var **name** (`apiKeyEnv`), never stored inline; keyless
  local endpoints (for example Ollama) omit it. Missing `provider` preserves
  the OpenAI-compatible default. When `apiKeyEnv` is present, it must name a
  non-empty environment variable at startup.
...
| `CHATZ_UPSTREAMS` | *(empty)* | JSON array of upstreams with `provider` set to `openai` or `anthropic`. Optional `models` entries attach aliases; token limits; tools/reasoning/vision/files support; expected first-token latency; and input/output prices only to IDs discovered from that upstream. Prices are integer currency-smallest-units per million tokens (`amountSmallestUnit` + uppercase ISO currency), never floats. Empty ⇒ built-in OpenAI upstream keyed on `OPENAI_API_KEY`. |
...
| `OPENAI_API_KEY` | *(empty)* | Key for the built-in OpenAI upstream (referenced by name from `apiKeyEnv`). |
| `ANTHROPIC_API_KEY` | *(empty)* | Conventional key name for an Anthropic upstream referenced by `apiKeyEnv`. |
