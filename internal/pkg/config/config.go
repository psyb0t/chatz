...
// Defaults for the built-in upstream used when CHATZ_UPSTREAMS is unset.
const (
	defaultUpstreamName       = "openai"
	defaultUpstreamBaseURL    = "https://api.openai.com/v1"
	defaultUpstreamAPIKeyEnv  = "OPENAI_API_KEY" //nolint:gosec // env var name
	iso4217CurrencyCodeLength = 3
	tokenPriceDirectionInput  = "input"
	tokenPriceDirectionOutput = "output"
)

// UpstreamProvider identifies the protocol driver used by an LLM upstream.
...
type Config struct {
	...
	// UpstreamsJSON is a JSON array of Upstream. Empty falls back to a single
	// OpenAI upstream keyed on OPENAI_API_KEY.
...
// Upstreams returns the configured upstreams, or the built-in OpenAI default
// when CHATZ_UPSTREAMS is unset.
func (c Config) Upstreams() ([]Upstream, error) {
	if c.UpstreamsJSON == "" {
		return []Upstream{{
			...
			APIKeyEnv: defaultUpstreamAPIKeyEnv,
		}}, nil
	}
...
func validateConfiguredUpstream(
	...
	if upstream.Provider == "" {
		upstream.Provider = UpstreamProviderOpenAI
	}
