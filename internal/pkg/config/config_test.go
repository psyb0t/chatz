...
func TestConfig_Upstreams(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name:          "empty falls back to default openai",
			json:          "",
			wantLen:       1,
			wantFirstName: defaultUpstreamName,
			wantProvider:  UpstreamProviderOpenAI,
		},
		{
			name:          "single upstream",
			json:          `[{"name":"ollama","baseUrl":"http://x:11434/v1"}]`,
			wantLen:       1,
    ...
    {"id":"analysis","alias":"Deep","contextWindow":128000,
     ...
     "outputPricePerMillionTokens":{"amountSmallestUnit":60,"currency":"USD"}}
		...
		},
		{
			...
			json:    `[{"name":"gateway","models":[{}]}]`,
			wantErr: true,
		},
		{
			name:    "duplicate model metadata errors",
			json:    `[{"name":"gateway","models":[{"id":"a"},{"id":"a"}]}]`,
			wantErr: true,
		},
		{
...
func TestConfig_Upstreams_APIKeyEnvironment(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			json: `[
  {"name":"gateway","apiKeyEnv":"CHATZ_TEST_UPSTREAM_KEY"}
]`,
		},
		{
			name: "referenced key is missing",
			json: `[
  {"name":"gateway","apiKeyEnv":"CHATZ_TEST_MISSING_UPSTREAM_KEY"}
]`,
			wantErr: true,
		},
	}
