<script lang="ts">
  ...
  import { COMP_TIME_SERIES_CHART, DATA_JR_TYPE } from "$lib/common/render";
  import {
    CHART_MARGIN_BOTTOM,
    CHART_MARGIN_LEFT,
    CHART_MARGIN_RIGHT,
  ...
  interface PositionedSeries {
    name: string;
    segments: NumericPoint[][];
  }
  ...
  const { props }: BaseComponentProps<TimeSeriesChartProps> = $props();
  ...
  function shortSummaryLabel(value: string): string {
    if (value.length <= MAX_SUMMARY_LABEL_LENGTH) {
      return value;
    }

    return `${value.slice(0, MAX_SUMMARY_LABEL_LENGTH - 1)}…`;
  }
  ...
  function normalizeSeries(value: unknown): SafeSeries[] {
    ...
    return value.slice(0, MAX_SERIES).flatMap((candidate) => {
      ...
      return [{ name: safeText(record.name), segments }];
    });
  }
  ...
  function xCoordinates(points: readonly SafePoint[]): Map<string, number> {
    ...
    const allDates = parsed.every(Number.isFinite);
  ...
  function summarizeChart(
    ...
    if (allChartPoints.length === 0) {
      return "No finite time-series data is available.";
    }
  ...
  const yDomain = $derived(extent(points.map((point) => point.y)));
  ...
  const positionedSeries = $derived<PositionedSeries[]>(
    safeSeries.map((item) => ({
      name: item.name,
...
<section
  ...
  {...{ [DATA_JR_TYPE]: COMP_TIME_SERIES_CHART }}
>
  {#if title}
    <h3>{title}</h3>
  {/if}
  <p class="jr-chart__sr">{summary}</p>

  {#if points.length === 0}
    <div class="jr-chart__empty" role="status">No finite time-series data</div>
  {:else}
    <svg
      viewBox={`0 0 ${CHART_VIEWBOX_WIDTH} ${CHART_VIEWBOX_HEIGHT}`}
      {height}
      role="img"
      aria-label={title || "Time series values"}
    >
      <title>{title || "Time series values"}</title>
    ...
    {#if safeSeries.length > 1 || safeSeries.some((item) => item.name)}
      <ul class="jr-chart__legend" aria-label="Series legend">
        {#each safeSeries as item, index (index)}
          <li>
            <span
              class="jr-chart__swatch"
              class:jr-chart__swatch--dashed={seriesDash(index)}
...
<style>
  ...
  .jr-chart__legend li span:last-child {
    overflow: hidden;
