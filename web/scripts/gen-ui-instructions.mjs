// Generates the LLM-facing GenUI (json-render) system-prompt instructions from
// chatz's component catalog — the SAME catalog as src/lib/render/catalog.ts,
// redeclared here in plain ESM so it can run under node without the SvelteKit /
// TS toolchain. The output (stdout) is captured to a .gen.txt that the Go
// backend embeds and appends to every chat turn's system prompt.
//
// Keep this catalog in lockstep with src/lib/render/catalog.ts + common/render.ts.
// Lives under web/ so bare imports resolve web/node_modules.
...
const catalog = schema.createCatalog({
  components: {
    ...
    Stat: {
      ...
      slots: [],
      description:
        "A single metric: label, value, optional unit, and an optional signed delta (positive renders up/ok, negative down/crit).",
      example: {
        id: null,
        label: "Occupancy",
        value: "92",
...
if (process.argv.includes("--list-components")) {
  process.stdout.write(`${JSON.stringify(catalog.componentNames)}\n`);
} else {
  process.stdout.write(
    catalog.prompt({
      ...
      customRules: [
        ...
        "NEVER emit a prop or list entry with empty, null, or whitespace-only content (the `id` prop is the sole exception — it is null by contract). Omit the key or the entry instead.",
        "Keep UI minimal and purposeful — use the most specific component that fits. Use charts only when they clarify a comparison, trend, distribution, relationship, or composition better than prose or a small table. Ground every value in real data from the conversation; never fabricate sample data.",
      ],
    }),
  );
}
