...
// Package api drives a real headless browser (stealthy-auto-browse) against the
...
// newStack brings up a fresh api stack + browser for one test and registers
...
func newStack(
	t *testing.T,
	apiOpts testinfra.APIOptions,
	browserOpts testinfra.BrowserOptions,
) (*testinfra.APIStack, *testinfra.BrowserClient) {
	t.Helper()

	setupCtx := context.Background()

	stack, err := testinfra.SetupAPI(setupCtx, apiOpts)
	require.NoError(t, err, "bring up api stack")
	t.Cleanup(func() {
...
func setupAdmin(
	...
	require.NoError(t, client.Fill(ctx, setupUsernameSel, testinfra.APIUser))
	require.NoError(t, client.Fill(ctx, setupPasswordSel, testinfra.APIPass))
